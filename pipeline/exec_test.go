...
func TestPromise(t *testing.T) {
	promise := pipeline.Run(os.Stdin, os.Stdout, "echo", "hello")
	err := promise.Wait()
	if err != nil {
...
func TestPromiseError(t *testing.T) {
	promise := pipeline.Run(os.Stdin, os.Stdout, "asdf", "ghjk")
	err := promise.Wait()
	if err == nil {
...
func TestPromiseInput(t *testing.T) {
	promise := pipeline.Run(os.Stdin, os.Stdout, "man", "watch")
	err := promise.Wait()
	if err != nil {
		t.Errorf("Error while executing promise %v", err)
	}
...
func TestPromiseWithoutWait(t *testing.T) {
	...
	promise := pipeline.Run(os.Stdin, os.Stdout, "watch", "-n", "1", "echo", "hello")
